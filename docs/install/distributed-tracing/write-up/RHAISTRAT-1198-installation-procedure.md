...
1. **In-memory storage** — Traces are stored in memory and are lost if the pod restarts. For production deployments, use `TempoStack` with object storage (e.g., S3 or ODF) instead.
...
3. **Multi-tenancy** — By default multi-tenancy is not enabled. OpenShift emits a warning about this during creation. Enabling multi-tenancy (`multitenancy.enabled: true`, `mode: openshift`) adds auth to the ingest and query paths, but the Jaeger UI currently does not send the required `X-Scope-OrgID` tenant header, making the UI inaccessible. For demo purposes, leaving multi-tenancy disabled is recommended.
