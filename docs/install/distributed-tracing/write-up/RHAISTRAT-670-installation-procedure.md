...
Because **we only allow for one inference stack per namesapce** at this time, we will create the `distributed-tracing` namespace, although this should work for any namespace provided you adjust the manifests accordingly.
...
Now using project "distributed-tracing" on server "https://api.qfpda-by6c4-2cb.oiah.p3.openshiftapps.com:443".
```
...
```console
llminferenceservice.serving.kserve.io/distributed-tracing-llama created
```
...
for m in num_requests_running num_requests_waiting kv_cache_usage_perc; do
    curl -sk -G -H "Authorization: Bearer $TOKEN" "https://$THANOS/api/v1/query" \
      --data-urlencode "query=vllm:${m}{namespace=\"distributed-tracing\"}" | \
      jq '.data.result[0]'
done
```
...
```json
{
  "metric": {
    ...
    "job": "distributed-tracing/kserve-llm-isvc-vllm-engine",
    "llm_isvc_component": "workload",
    "llm_isvc_name": "distributed-tracing-llama",
    "llm_isvc_role": "both",
    "model_name": "Qwen/Qwen2.5-7B-Instruct",
    "namespace": "distributed-tracing",
    "pod": "distributed-tracing-llama-kserve-56b64d69d-b297x",
...
for m in wva_desired_replicas wva_current_replicas wva_desired_ratio; do
  curl -sk -G -H "Authorization: Bearer $TOKEN" "https://$THANOS/api/v1/query" \
    --data-urlencode "query=${m}{exported_namespace=\"distributed-tracing\"}" \
    | jq '.data.result[0]'
done
```
...
```json
{
  "metric": {
    ...
    "endpoint": "https",
    "exported_namespace": "distributed-tracing",
    ...
    "variant_name": "distributed-tracing-llama-kserve-va"
  },
  "value": [
    1775002469.694,
    "1"
...
```bash
k get va distributed-tracing-llama-kserve-va
NAME                                  TARGET                             MODEL                      OPTIMIZED   METRICSREADY   AGE
distributed-tracing-llama-kserve-va   distributed-tracing-llama-kserve   Qwen/Qwen2.5-7B-Instruct   1           True           81m

k get scaledObject -n distributed-tracing
NAME                                    SCALETARGETKIND      SCALETARGETNAME                    MIN   MAX   READY   ACTIVE   FALLBACK   PAUSED   TRIGGERS     AUTHENTICATIONS            AGE
distributed-tracing-llama-kserve-keda   apps/v1.Deployment   distributed-tracing-llama-kserve   1     5     True    True     Unknown    False    prometheus   ai-inference-keda-thanos   83m
```
...
NS="${1:-distributed-tracing}"
ISVC="${2:-distributed-tracing-llama}"
CONCURRENCY="${3:-200}"
REQUESTS="${4:-5000}"
...
URL:         https://distributed-tracing-gateway-data-science-gateway-class.distributed-tracing.svc.cluster.local/distributed-tracing/distributed-tracing-llama/v1/chat/completions
Concurrency: 200
Requests:    5000
...
```console
NAME                                                              READY   STATUS    RESTARTS      AGE
distributed-tracing-gateway-data-science-gateway-class-66c4qvpb   1/1     Running   0             81m
distributed-tracing-llama-kserve-56b64d69d-b297x                  1/1     Running   0             106m
distributed-tracing-llama-kserve-56b64d69d-kvvmc                  1/1     Running   0             10m
distributed-tracing-llama-kserve-56b64d69d-m78nz                  1/1     Running   0             7m1s
distributed-tracing-llama-kserve-router-scheduler-77cd5549p4w95   2/2     Running   0             106m
